...
- **One source of truth.** A `Definition` declares the steps, fields, per-record visibility,
...
class ClosingForm::Definition < EmeraldCascade::Definition
  ...
  step :photos, title: 'Packing sheet photos', partial: 'photos', own_form: true,
       visible_when: ->(s) { s.photos_step? }
  step :review, title: 'Review & submit', partial: 'review'
end
```

- Steps are kept in declaration order; `visible_when` only *omits* a step for some records.
- `partial:` renders a bespoke partial instead of the generic field list; add `own_form: true`
  when that partial renders its own form(s) (e.g. an upload step) rather than the shared PATCH
  form — the host's show template reads `step.renders_own_form?` to decide how to wrap it.
...
Mix into any model with a string `state` column to get a resumable, definition-driven step
lifecycle:
...
- **Navigation helpers** translate the persisted `state` (a superset step) to the per-record
  *visible* subset: `visible_steps`, `visible_step_keys`, `next_step_of` / `prev_step_of`,
  `begin_flow!`, `advance_from!`, `goto_step!`, etc.
...
lookup, route helpers and a few step-specific hooks.
...
- `EmeraldCascade::Flow` - shared base: the `@submission` guards (`require_submission!`,
  ...
  a before_action and defines its routes (`submission_url`, `step_url(step)`). Include it in
  the host's base controller.
- `EmeraldCascade::StepFlow` - one step at a time: `show`/`update`, per-step save + validate
  (`valid_step?`), advance, and the review-step submit (`valid_for_submit?`). Host hooks:
...
class ClosingForm::BaseController < ApplicationController
  ...
  def step_url(step) = closing_form_step_path(step:, ...)
end
...
`valid_step?(key)` and `valid_for_submit?`. Validation stays host-owned because a step may
...
- `steps/_nav_busy_script` - grays out/disables the `.emerald-actions` nav the moment it's
  pressed (double-submit/double-visit guard), restores it if a submit fails without replacing the
  page (`turbo:submit-end` failure), and swallows repeat Back presses while busy.
- `attachments/_poll_script` - a Turbo-frame self-poller while a `poll_marker` is present. Locals:
  `poll_marker`, `frame_id`, `interval`, and optional `uploader_selector` (a CSS selector,
  relative to the frame, whose click opens a native file picker). Passing `uploader_selector`
  pauses polling while that picker is open, so a background reload can't replace the
  `<input type=file>` and drop the selection (notably on mobile); it also skips a beat whenever a
  request targeting the frame is already in flight rather than aborting it.
